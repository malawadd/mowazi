...
export default function PublicPaymentForm({ paymentLink }: { paymentLink: PublicPaymentLink }) {
  ...
  const previewPayment = async () => {
    ...
    try {
      ...
      if (!workingIntentId) {
        ...
        if (!intent) {
          throw new Error("Could not create a payment intent.");
        }
  ...
  return (
    <Panel title="Deposit to account" description={`Recipient: ${paymentLink.recipientName}`} tone="sky">
      <div className="stack-list">
        ...
        <div className="inline-actions">
          <button className="secondary-button" type="button" disabled={busy !== null} onClick={connectPayer}>
            {busy === "connect" ? "Connecting..." : address ? "Refresh payer wallet" : "Connect wallet"}
          </button>
          <button
